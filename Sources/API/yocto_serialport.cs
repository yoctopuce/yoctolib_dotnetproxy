...
//--- (end of generated code: YSerialPort class start)
    ...
    public delegate void YSnoopingCallback(YSerialPort obj, YSnoopingRecord rec);

    protected static void yInternalEventCallback(YSerialPort obj, String value)
    {
     ...
     * </para>
     */
    public virtual List<YSnoopingRecord> snoopMessagesEx(int maxWait, int maxMsg)
    {
        string url;
        byte[] msgbin = new byte[0];
        List<byte[]> msgarr = new List<byte[]>();
        ...
        idx = 0;
        while (idx < msglen) {
            res.Add(new YSnoopingRecord(YAPI.DefaultEncoding.GetString(msgarr[idx])));
            idx = idx + 1;
        }
        return res;
    }
     ...
     * </param>
     */
    public virtual int registerSnoopingCallback(YSnoopingCallback callback)
    {
        if (callback != null) {
    ...
    public virtual int _internalEventHandler(string advstr)
    {
        string url;
        byte[] msgbin = new byte[0];
        List<byte[]> msgarr = new List<byte[]>();
